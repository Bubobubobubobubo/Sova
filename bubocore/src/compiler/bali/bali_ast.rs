...
lazy_static! {
    static ref NOTE_MAP: HashMap<String, i64> = {
        ...
        // Define note names and their base values
        let notes = [
        ...
        // Generate all octaves from -2 to 8
        for octave in -2..=8 {
            for (note, base_value) in notes.iter() {
                let midi_value = base_value + ((octave + 2) * 12);
                
                // Skip invalid MIDI values
                if midi_value < 0 || midi_value > 127 {
                    continue;
                }

                // Generate all possible note name variations
                let variations = [
                    format!("{}{}", note, octave),
                    format!("{}{}#", note, octave),
                    format!("{}#{}", note, octave),
                    format!("{}{}b", note, octave),
                    format!("{}b{}", note, octave),
                ];

                // Add special case for octave 3 (default octave)
                if octave == 3 {
