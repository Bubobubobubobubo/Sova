<script lang="ts">
	...
	$effect(() => {
		window.addEventListener('pane-drag-move', handleDragMove as EventListener);
		return () => {
			window.removeEventListener('pane-drag-move', handleDragMove as EventListener);
		};
	});
</script>
...
{#if $paneDragState}
	...
	>
		{$paneDragState.viewType ? viewTitles[$paneDragState.viewType] : 'Empty'}
	</div>
{/if}
