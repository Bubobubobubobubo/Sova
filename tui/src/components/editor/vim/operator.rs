...
fn execute_characterwise_yank(textarea: &mut TextArea, range: TextRange) {
    ...
    textarea.move_cursor(CursorMove::Jump(original_cursor.0 as u16, original_cursor.1 as u16));
}
...
fn select_range(textarea: &mut TextArea, range: TextRange) {
    textarea.move_cursor(CursorMove::Jump(range.start_row as u16, range.start_col as u16));
    textarea.start_selection();
...
pub fn paste_before(app: &mut App) -> OperatorResult {
    match paste_text(&mut app.editor.textarea, true, app.editor.vim_state.yank_register.yank_type) {
...
pub fn paste_after(app: &mut App) -> OperatorResult {
    match paste_text(&mut app.editor.textarea, false, app.editor.vim_state.yank_register.yank_type) {
        Ok(_) => OperatorResult::Success(Mode::Normal),
        Err(e) => OperatorResult::Error(e),
    }
}
